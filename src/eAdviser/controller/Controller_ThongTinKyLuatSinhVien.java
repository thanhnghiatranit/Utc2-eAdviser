...
public class Controller_ThongTinKyLuatSinhVien extends ThongTinKyLuatSinhVien implements ZEController,ServletRequestAware,ServletResponseAware{
	...
	public String exportData() throws IOException {
		...
		cell.setCellValue("Danh sách kỷ luật sinh viên ");
		...
		cell.setCellValue("Thời gian kỷ luật");
		cell.setCellStyle(style);
		...
		// Data
				...
				ArrayList<ThongTinKyLuatSinhVien> list_thongtinkyluat = new ArrayList<>();
				
				String maSo=session.getAttribute("maSo").toString();
				if (maSo!=null && !maSo.isEmpty()) {
					list_thongtinkyluat = dao_thongtinkyluat.listByColumnLike("soCoVanHocTap", maSo);
				}
				else
				{
					list_thongtinkyluat = dao_thongtinkyluat.listAll();
				}
		int stt = 0;
		for (ThongTinKyLuatSinhVien thongTinKyLuatSinhVien : list_thongtinkyluat) {
			rownum++;
			stt++;
			row = sheet.createRow(rownum);
			...
			cell = row.createCell(1, CellType.STRING);
			cell.setCellValue(thongTinKyLuatSinhVien.getMaThongTinKyLuatSinhVien());

			cell = row.createCell(2, CellType.STRING);
			cell.setCellValue(thongTinKyLuatSinhVien.getSinhVien().getMaSinhVien());

			 cell = row.createCell(3, CellType.STRING);
			 cell.setCellValue(thongTinKyLuatSinhVien.getSinhVien().getHoDem());
			
			 cell = row.createCell(4, CellType.STRING);
			 cell.setCellValue(thongTinKyLuatSinhVien.getSinhVien().getTen());

			cell = row.createCell(5, CellType.STRING);
			cell.setCellValue(thongTinKyLuatSinhVien.getNoiDungKyLuat());

			cell = row.createCell(6, CellType.STRING);
			cell.setCellValue(Util_Date.dateToString2(thongTinKyLuatSinhVien.getThoiGianKyLuat()));

			cell = row.createCell(7, CellType.STRING);
			cell.setCellValue(thongTinKyLuatSinhVien.getGhiChu());
		}
		...
		String fileName = "Danh sach ky luat sinh vien.xls";
